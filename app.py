...
.mol-name-box { background: #292524; border: 1px solid #44403c; border-radius: 8px; padding: 10px 14px; color: #fafaf9; font-size: 13px; font-weight: 500; margin-bottom: 4px; }
.mol-smiles-box { background: #1c1917; border: 1px solid #44403c; border-radius: 6px; padding: 6px 10px; color: #78716c; font-size: 10px; font-family: monospace; margin-bottom: 12px; word-break: break-all; }
...
def search_pubchem(name):
    try:
        url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/" + name.strip() + "/property/IsomericSMILES,IUPACName,MolecularFormula,MolecularWeight/JSON"
...
def fetch_chembl_data():
    ...
    targets = ["CHEMBL202", "CHEMBL204", "CHEMBL1827", "CHEMBL261",
...
def build_model():
    ...
    X_train, X_cal = X[idx[:n-cal_size]], X[idx[n-cal_size:]]
    y_train, y_cal = y[idx[:n-cal_size]], y[idx[n-cal_size:]]
        ...
        if drug_name:
            ...
            if result:
                ...
                st.markdown(
                    ...
                    '<div class="search-name">Found: ' + result["name"].title() + '</div>'
                    '<div class="search-smiles">Formula: ' + result["formula"] + ' · SMILES: ' + smiles[:80] + ('...' if len(smiles) > 80 else '') + '</div>'
                    '</div>',
                    unsafe_allow_html=True
                )
            elif drug_name:
    ...
    if smiles:
        ...
        try:
            ...
            if mol is None:
                ...
                with lc1:
                    dot1 = "lip-dot-pass" if rule1 else "lip-dot-fail"
                    dot2 = "lip-dot-pass" if rule2 else "lip-dot-fail"
                    st.markdown('<div class="lip-row"><div class="' + dot1 + '"></div><div class="lip-text">Molecular weight ≤ 500</div><div class="lip-val">' + str(mw) + '</div></div>', unsafe_allow_html=True)
                    st.markdown('<div class="lip-row"><div class="' + dot2 + '"></div><div class="lip-text">LogP ≤ 5</div><div class="lip-val">' + str(logp) + '</div></div>', unsafe_allow_html=True)
                with lc2:
                    dot3 = "lip-dot-pass" if rule3 else "lip-dot-fail"
                    dot4 = "lip-dot-pass" if rule4 else "lip-dot-fail"
                    st.markdown('<div class="lip-row"><div class="' + dot3 + '"></div><div class="lip-text">H-bond donors ≤ 5</div><div class="lip-val">' + str(hbd) + '</div></div>', unsafe_allow_html=True)
                    st.markdown('<div class="lip-row"><div class="' + dot4 + '"></div><div class="lip-text">H-bond acceptors ≤ 10</div><div class="lip-val">' + str(hba) + '</div></div>', unsafe_allow_html=True)
                ...
                with st.expander("Feature importance — what drove this prediction?"):
                    ...
                    for name, imp in pairs:
                        ...
                        st.markdown(
                            ...
                            '<div class="feat-track"><div class="feat-fill" style="width:' + str(w) + '%"></div></div>'
